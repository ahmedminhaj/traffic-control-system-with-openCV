...
loc = os.path.abspath('')
inputVideoList = ["625_201709141306.mp4","625_201709301058.mp4","625_201708101531.mp4","625_201709042122.mp4"];
countVehicle = [];
countCar = 0;

for i in range (len(inputVideoList)):

	#inputFile = loc+'/inputs/625_201709141356.mp4'
	inputFile = loc+'/inputs/'+inputVideoList[i]
	...
	while ret:    
		...
		if ret and frame_no < total_frames:
			...
			# get latest count
			countCar = car_counter.update_count(blobs, frame)
	...
	cap.release()
	out.release()
	countVehicle.append(countCar)
lane_number = []
for i in range (len(countVehicle)):
    lane_number.append(countVehicle[i])

for i in range (len(countVehicle)):
    print(lane_number[i])
countVehicle.sort()	
for i in range (len(countVehicle)):
	if countVehicle[3] == lane_number[i]:
		var=str(i+1)
...
if (var == '2'): #if the value is 0
    ArduinoSerial.write(b'2') #send 0
    print("Lane 2")
    time.sleep(1)
        
if (var == '3'): #if the value is 0
    ArduinoSerial.write(b'3') #send 0
    print("Lane 3")
    time.sleep(1)
              
if (var == '4'): #if the value is 0
    ArduinoSerial.write(b'4') #send 0
    print("lane4")
    time.sleep(1)
